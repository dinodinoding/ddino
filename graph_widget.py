# GUI 스크립트 파일 전체를 이 코드로 교체하세요. (최소 변경 버전)
# 변경점:
# 1) is_worker_running() 함수 추가
# 2) start_monitoring()에서 워커 즉시 실행 시, 이미 떠 있으면 실행 안 함
...
# >>> [추가] 현재 워커 동작 여부 확인 (정확 매칭)
def is_worker_running(exe_name: str) -> bool:
    try:
        # 헤더 없이 정확 매칭 필터
        ...
        txt = result.stdout.decode('cp949', errors='ignore').strip().lower()
        # "정보:"(KO)/"info:"(EN)로 시작하면 해당 프로세스 없음
        if not txt or txt.startswith(('정보:', 'info:')):
            return False
        return exe_name.lower() in txt
    except Exception:
        return False
# <<< [추가 끝]
...
class GUI_App(QWidget):
    ...
    def start_monitoring(self):
        ...
        try:
            ...
            # 4. 워커를 즉시 실행 (이미 떠있으면 실행 안 함)  >>> [변경]
            self.register_worker_autostart()
            if not is_worker_running(self.worker_exe_name):  # <<< 추가
                subprocess.Popen([worker_exe_path], creationflags=subprocess.DETACHED_PROCESS, close_fds=True)  # 기존 라인 유지
