...
# 변경점(한 가지): Start Monitoring 클릭 시 worker를 즉시 1회 실행(이미 떠 있으면 생략)
...
class GUI_App(QWidget):
    ...
    def start_monitoring(self):
        ...
        try:
            ...
            # 1) PowerShell 스크립트 파일 생성 (정확 매칭 확인 + 없을 때만 실행 + 작업디렉터리 지정)
...
'''
            ...
            # 5) [추가] Start 클릭 시 즉시 1회 실행 (이미 떠 있으면 실행 안 함)
            if not is_process_running(self.worker_exe_name):
                subprocess.Popen([worker_exe_path],
